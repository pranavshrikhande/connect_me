...
import com.code.facebookclone.repository.PostEntityRepository;
...
public class PostServiceImpl implements PostService {
    ...
    public List<Post> getPost() {
        ...
        posts= postEntities.stream().map((postEntity)->Post.builder().id(postEntity.getId()).timeStamp(postEntity.getTimeStamp()).email(postEntity.getEmail()).name(postEntity.getName()).post(postEntity.getPost()).image(postEntity.getImage()).profilePic(postEntity.getProfilePic()).build()).collect(Collectors.toList());
